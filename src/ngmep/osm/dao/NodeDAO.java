...
public class NodeDAO extends AbstractEntityDAO {
    ...
    public List<Node> getNodesByTag (String tagName, String value) throws SQLException {
        
        String query = QUERY_BASE + "where exists (select 1 from node_tags t where t.node_id = n.id and t.k = ?";
        ...
        List<Node> resultado = getNodes(rs);
        rs.close();
        ps.close();
    ...
    private Node getNode(ResultSet rs) throws SQLException {
        ...
        PreparedStatement ps2 = Database.getConnection().prepareStatement(get_QUERY_NODE_TAGS());
        ps2.setLong(1,id);
    ...
    public Node getNode(long id) throws SQLException{
        PreparedStatement ps = Database.getConnection().prepareStatement(QUERY_NODE);
        ps.setLong(1, id);
        ResultSet rs = ps.executeQuery();
        Node node = null;
        if (rs.next()){
